...
def setup():
    ...
    default_config_file = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'docker.ini')
    config_file = os.environ.get('DOCKER_CONFIG_FILE', default_config_file)
...
def list_groups():
    ...
    for host in hosts:
        ...
        try:
            ...
            client = docker.Client(base_url=host['base_url'],
        ...
        for container in containers:
            ...
            # Group containers by docker host
            groups[hostname].append(name)

            ### Custom section
            if re.match('^docker-.*', name) is not None:

                # Group containers by Name suffix (docker-java-00 container -> java group)
                group_name = re.sub(r'docker-([a-zA-Z0-9\_\-]+)-([0-9]+)$', r'\1', name)
                # For each group_name we will generate the role group and the cluster group
                # For example:
                #   CONTAINER NAME || GROUP NAME
                #   docker-java-00 -> docker-java -> Used to target container
                #                  -> java        -> role group     # Used to apply correct inv vars
                #                  -> java-test   -> cluster group  # Used to apply correct inv vars
                custom_groups = [
                    "docker-%s" % group_name,
                    group_name,
                    "%s-test" % group_name
                ...
                special_custom_groups = [
                    # All kind of Druid instances are part of the parent druid group
                    dict(pattern='^druid-(?!pivot)', group_name='druid'),
                    dict(pattern='^druid-(?!pivot)', group_name='druid-test'),
                    dict(pattern='^druid-(?!pivot)', group_name='docker-druid'),

                    # Cassandra instances are also part of a few extra groups for maintenance
                    dict(pattern='^cassandra$', group_name='cleanup-group'),
                    dict(pattern='^cassandra$', group_name='compaction-group'),
                    dict(pattern='^cassandra$', group_name='repair-group'),

                    # All kind of Storm instances are part of the parent storm group
                    dict(pattern='^storm-(nimbus|supervisor)', group_name='storm'),
                    dict(pattern='^storm-(nimbus|supervisor)', group_name='storm-test'),
                    dict(pattern='^storm-(nimbus|supervisor)', group_name='docker-storm')
                ]
                ...
                for custom_group in custom_groups:
                    ...
                    if group_name != name:
                        groups[custom_group].append(name)

                    for special_group in special_custom_groups:
                        if re.match(special_group['pattern'], custom_group) is not None:
                            if special_group['group_name'] not in groups:
                                groups[special_group['group_name']] = []

                            if name not in groups[special_group['group_name']]:
                                groups[special_group['group_name']].append(name)

            ### End of custom section

            if running is True:
                groups['docker_running_containers'].append(name)
            else:
                groups['docker_stopped_containers'].append(name)

            groups['docker'].append(name)
